...
public class XML2 extends Base {
	...
	streamNode : for(Streamnode node1 : listofStreamsNodes){
		...
		/****************************************
		 * Parsing Second streamnode>> MPEG2TPTchecks
		 *****************************************/
		...
		parm :	for(Customchecks errorcustom1: errorsObj.getCustomchecks()) {
				for(ChecksType errortypes : errorcustom1.getDecoded3DvideochecksOrGIFimagechecksOrCanopusvideochecks()){
					if(errortypes.getName().equals("MPEG2 Transport")){
						mpeg2=1;
						Logger.accessLogger().addReport("streamnode>>errors>>customchecks>>MPEG2TPTchecks(name=MPEG2 Transport)::" + streamnode1.getFolderName(), 1,
								"EQ", mpeg2, 1);
						System.out.println(errortypes.getError());
						for(generated.Error firsterror: errortypes.getError()){
						if(firsterror.getId().equalsIgnoreCase("MPEG2TS.0018.01")){
							for(generated.Params parmNames: firsterror.getParams()){
								for(generated.Param firstParmName: parmNames.getParam()){
									if(firstParmName.getName().equalsIgnoreCase("PCRPIDS")){
										parmname = 1;
										Logger.accessLogger().addReport("streamnode>>errors>>customchecks>>MPEG2TPTchecks(name=MPEG2 Transport)>>error(id=MPEG2TS.0018.01)>>Parm(name=PCRPIDS)::" + streamnode1.getFolderName(), 1,
												"EQ", parmname, 1);
										break parm;
									}
								}
